...
<script>
	$(function(){
    	$("#UserResist").on("click", function(){
       		location.href='${cp }/member/registUser';
    	});
 	});
	...
	$(function(){
		...
		$('#perPageNum').val('${param.pageSize}');
		
		
	})
	
	function sort(){
		var langSelect = document.getElementById("perPageNum");
		var selectValue = langSelect.options[langSelect.selectedIndex].value;
		
		if(selectValue!=3&&selectValue!=5&&selectValue!=7){
			location.href='${cp}/member/pagingUser?pageSize='+5;
		}
		else{
			location.href='${cp}/member/pagingUser?pageSize='+selectValue;
		}	
	}
	
	function search(){
		var search = document.getElementById("search");
		var field = search.options[search.selectedIndex].value;
		...
		location.href='${cp}/member/pagingUser?field='+field + '&keyword='+keyword + '&pageSize='+selectValue;
	}
...
<body class="hold-transition sidebar-mini">
	<form id="frm" action="${cp }/member/userInfo" >
				...
				<section class="content">
					<div class="card">
						<div class="card-header with-border">
							<button type="button" id="UserResist" class="btn btn-primary" ">회원등록</button>
							<div id="keywords" class="card-tools" style="width: 550px;">
								<div class="input-group row">
									<!-- sort num -->
									
									<select class="form-control col-md-3" name="perPageNum" id="perPageNum" onchange="sort()">
									...
									<!-- search bar -->
									<select class="form-control col-md-3" name="search" id="search" >
										<option value="" >검색구분</option>
										<option value="i">아이디</option>
										<option value="m">이름</option>
										<option value="a">별명</option>
									</select> 
									<input class="form-control" type="text" name="keyword" id="keyword" placeholder="검색어를 입력하세요." value="${keyword }"> 
									<span class="input-group-append">
										<button class="btn btn-primary" type="button" id="searchBtn" data-card-widget="search" onclick="search()">
                   ...
                   <nav aria-label="member list Navigation">
                     <ul class="pagination justify-content-center m-0">
                         <li class="page-item">
                           <a class="page-link" href="${cp}/member/pagingUser?page=1&pageSize=${pageVo.pageSize}"><i class="fas fa-angle-double-left"></i></a>
                        </li>
                        <li class="page-item">
                           <a class="page-link" href="${cp}/member/pagingUser?page=${pageVo.page -1 }&pageSize=${pageVo.pageSize}"><i class="fas fa-angle-left"></i></a>
                        </li>
                        <c:forEach begin="1" end="${pagination }" var="i">
                           <c:choose>
                              <c:when test="${pageVo.page == i}"><li class="page-item active"><a class="page-link" href="#">${i }</a></li></c:when>
                              <c:otherwise><li class="page-item"><a class="page-link" href="${cp}/member/pagingUser?page=${i }&pageSize=${pageVo.pageSize }">${i }</a></li></c:otherwise>
                           </c:choose>                        
                        </c:forEach>
                         <li class="page-item">
                           <a class="page-link" href="${cp}/member/pagingUser?page=${pageVo.page +1 }&pageSize=${pageVo.pageSize}"><i class="fas fa-angle-right"></i></a>
                        </li>
                        <li class="page-item">
                           <a class="page-link" href="${cp}/member/pagingUser?page=${pagination }&pageSize=${pageVo.pageSize }"><i class="fas fa-angle-double-right"></i></a>
                        </li>
                     </ul>
                     </nav>
                  </div> 
